...
def build_fetcher_factory(
    ...
    build_fetcher_with_client = lambda endpoint: build_fetcher(endpoint, api_client)  # type: ignore

    factory.register(
        GarminMetricId.SLEEP, build_fetcher_with_client(GarminEndpoint.DAILY_SLEEP)
    )
    factory.register(
        GarminMetricId.RHR, build_fetcher_with_client(GarminEndpoint.DAILY_RHR)
    )
    factory.register(
        GarminMetricId.SLEEP_SCORE,
        build_fetcher_with_client(GarminEndpoint.DAILY_SLEEP_SCORE),
    )
    factory.register(
        GarminMetricId.BB, build_fetcher_with_client(GarminEndpoint.DAILY_BB)
    )
    factory.register(
        GarminMetricId.HRV, build_fetcher_with_client(GarminEndpoint.DAILY_HRV)
    )
    factory.register(
        GarminMetricId.STRESS, build_fetcher_with_client(GarminEndpoint.DAILY_STRESS)
    )
...
def build_fetcher(endpoint: GarminEndpoint, client: GarminApiClient) -> Fetcher:
    def fetcher(period: DatePeriod) -> ApiResponse:
        data: JsonResponseType | None = client.get_data(endpoint, period)
